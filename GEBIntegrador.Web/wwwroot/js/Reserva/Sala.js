...
function seleccionarPiso() {
    var pisosLista = document.getElementById('pisoDropdownItems');
    var pisoId = document.getElementById('pisoDropdownItems').value;
    var textoPiso = pisosLista.options[pisosLista.selectedIndex].text;

    if (pisoId != "") {

        //var imagePath = '/images/niveles/' + textoPiso + '.png';
        //document.getElementById('imagenNivel').src = imagePath;
        var pdfPath = '/images/niveles/' + textoPiso + '.pdf';
        document.getElementById('pdfViewer').src = pdfPath;

        var data = new Object()
        data.idPiso = pisoId;

        ajax.get("/Recursos/ObtenerSalas", data).then((d) => {
            procesarRespuestaListaSalas(d);
        });
    }
}

function procesarRespuestaListaSalas(d) {
    if (d.success) {
        if (d.data != null) {
            var lista = d.data;

            var salaDropdown = document.getElementById('salaDropdownItems');
            salaDropdown.innerHTML = ""; // Limpiar el select antes de agregar opciones

            // Agregar la opción predeterminada
            var defaultOption = document.createElement('option');
            defaultOption.value = "";
            defaultOption.text = "--Elija una sala--";
            salaDropdown.appendChild(defaultOption);
            ...
            lista.forEach(p => {
                ...
                salaDropdown.appendChild(option);
            });

            // Agregar evento al cambio de selección en el dropdown de salas
            salaDropdown.addEventListener('change', function () {
                var selectedIndex = salaDropdown.selectedIndex;
...
function mostrarCapacidadSala(capacidad) {
    var capacidadInput = document.getElementById('capacidadSala');
    capacidadInput.value = capacidad + " Personas";
}
...
function realizarReserva() {
    var salaId = document.getElementById('salaDropdownItems').value;
...
function procesarRespuestaCreacionReserva(d) {
    if (d.success) {
        msg.msgSuccess(d.message).then((result) => {
            msg.msgSuccess("Reserva de sala de reunión exitosa");
...
window.seleccionarPiso = seleccionarPiso;