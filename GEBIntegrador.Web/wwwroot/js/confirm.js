...
(function (factory) {
    ...
    w.Jconfirm.prototype = {
        ...
        _startCountDown: function () {
            ...
            var fecha = that._localstoragemanager(opt[3]);
            var end = new Date(that._addtime(fecha,opt[1], opt[2]));
            if (opt.length !== 4) {
                console.error('Invalid option for autoClose. example \'cerrar|desdeFecha|intervalo|unidad("min","seg")\'');
                return false;
            }
            ...
            var seconds = (that._formatday(end));
            this.$cd = $('<span class="countdown"> (' + seconds + ')</span>')
                .appendTo(this['$_' + button_key]);

            this.autoCloseInterval = setInterval(function () {
                seconds = (that._formatday(end));
                that.$cd.html(' (' + seconds + ') ');
                if (seconds <= 0) {
                    window.localStorage.removeItem(opt[3]);
                    that['$$' + button_key].trigger('click');
                    that._stopCountDown();
                }
            }, 1000);
        },
        _localstoragemanager: function (storage) {
            var that = this;
            var local = window.localStorage.getItem(storage);
            if (typeof (local) == "undefined" || local == null) {
                var _time = that._timenow();
                window.localStorage.setItem(storage, _time);
                return _time;
            } else {
                return local;
            }
        },
        _formatday: function (end) {
            var now = new Date();
        ...
        _addtime: function (fecha, adicion, key) {
            var v = new Date(fecha);
            switch (key) {
                case 'mil':
                    return v.setMinutes(v.getMilliseconds() + adicion);
                case 'seg':
                    return v.setMinutes(v.getSeconds() + adicion);
                case 'min':
                    return v.setMinutes(v.getMinutes() + adicion);
                case 'hor':
                    return v.setMinutes(v.getHours() + adicion);
                case 'dia':
                    return v.setMinutes(v.getDay() + adicion);
                case 'sem':
                    return v.setMinutes(7 + adicion);
                case '365':
                    return v.setMinutes(v.getFullYear() + adicion);
                default:
                    return v.setMinutes(v.getMinutes() + adicion);
            }
        },
